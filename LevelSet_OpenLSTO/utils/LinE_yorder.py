...
class FEAMeshQ4(object):
    ...
    def get_Mesh(self,lxy,exy):
        # following x, y order from m2do: x firsti
	# 9/13: changed to y-first 
        (lx,ly) = lxy
        (ex,ey) = exy
        (nx,ny) = (ex+1, ey+1)        
	...
	Nodes[:,0] = x.flatten()#order='F')
        ...
        for xx in range(0,ex):
            for yy in range(0,ey):
                Elements[eid,0] = ny*(xx)   + yy #nx*(yy)   + (xx)
                Elements[eid,1] = ny*(xx+1) + (yy)
                Elements[eid,2] = ny*(xx+1) + (yy+1)
                Elements[eid,3] = ny*(xx)   + (yy+1)
                eid += 1
