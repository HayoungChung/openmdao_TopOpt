...
from utils.gauss_quadrature import gauss_pts_dict


def get_coord_eval(num_nodes_x, num_nodes_y, quad_order, flatten=True):
    # get gauss points in x, y (scaled: 0~1)
...
def get_coord_tmp(num_cp_x, num_cp_y, order = 'xy'):
    # scaled coordinates of control point (min-max = 0-1)
    if order == 'xy':
        num_tmp_x = 2 * num_cp_x
        num_tmp_y = 2 * num_cp_y

        coord_tmp = np.zeros((num_tmp_y, num_tmp_x, 2))

        lins_x = np.linspace(0, 1, num_tmp_x)
        lins_y = np.linspace(0, 1, num_tmp_y)

        (xl_, yl_) = np.meshgrid(lins_x, lins_y)
        coord_tmp[:, :, 0] = xl_
        coord_tmp[:, :, 1] = yl_
    ...
    # '''
        ...
        # this is original ver. (yx direction)
        num_tmp_x = 2 * num_cp_x
        num_tmp_y = 2 * num_cp_y
