...
from utils.coords import get_coord_tmp


def get_bspline_mtx(coord_eval_x, coord_eval_y, num_cp_x, num_cp_y, kx=4, ky=4, node_order='xy'):
    ...
    if node_order == 'yx':
        out0 = bisplev(coord_eval_x, coord_eval_y, tck).flatten() # evaluation of tck
    elif node_order == 'xy':
        out0 = bisplev(coord_eval_x, coord_eval_y, tck).flatten(order = 'F') # evaluation of tck
    
    for ind in range(num_cp):
        tck[2][ind] += h
    
        if node_order == 'yx':
            out = bisplev(coord_eval_x, coord_eval_y, tck).flatten() # evaluation of tck
        elif node_order == 'xy':
            out = bisplev(coord_eval_x, coord_eval_y, tck).flatten(order = 'F') # evaluation of tck
