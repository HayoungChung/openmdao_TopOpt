...
from components.parametrization_comp import ParametrizationComp
from components.averaging_comp import AveragingComp
...
from utils.coords import get_coord_eval, get_coord_tmp
...
class FEM2DParamGroup(Group):
    ...
    def setup(self):
        ...
        gpt_mesh = np.zeros((coord_eval_y.shape[0], coord_eval_x.shape[0], 2))
        ''' Dr. Hwang
        ...
        # below: xy order (openLSTO)
        '''
        gpt_mesh[:, :, 0] = np.outer(np.ones(coord_eval_y.shape[0]),coord_eval_x)
        gpt_mesh[:, :, 1] = np.outer(coord_eval_y, np.ones(coord_eval_x.shape[0]))
        ...
        else: # averaging again after penalization
            self.connect('parametrization_comp.y', 'heaviside_comp.x')
            comp = HeavisideComp(num=(num_nodes_x - 1) * (num_nodes_y - 1) * quad_order ** 2)
            self.add_subsystem('heaviside_comp', comp)
            self.connect('heaviside_comp.y', 'penalization_comp.x')
        ...
        comp = StatesComp(fem_solver=fem_solver, 
                          ...
                          isSIMP = False, quad_order = quad_order, gpt_mesh = gpt_mesh)
        # comp = StatesComp(
