...
def _plot(nodes, elem, axes, color='k'):
...
def plot_contour(mesh, field, plot_boundary=False, plot_fill=False, plot_patch=False):
    if plot_fill:
        ...
        if len(mesh.shape) == 2:
            ...
            y1 = np.min(mesh[:, 1])
            y2 = np.max(mesh[:, 1])
            plt.imshow(field, cmap='Greys', extent=[x1, x2, y1, y2]) #, zorder=2)
        if len(mesh.shape) == 3:
            x1 = np.min(mesh[:, :, 0])
            x2 = np.max(mesh[:, :, 0])
            y1 = np.min(mesh[:, :, 1])
    ...
    if plot_boundary:
        ...
        if len(mesh.shape) == 2:
            length_x = np.max(mesh[:, 0])
            length_y = np.max(mesh[:, 1])
            
            plt.subplot(2, 1, 2)
            plt.contour(mesh[:, 0], mesh[:, 1], field.T, levels=[0]) #, zorder=3)
