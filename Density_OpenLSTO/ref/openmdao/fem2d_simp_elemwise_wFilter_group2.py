...
# filter is imposed after penalization
import numpy as np
...
from fem2d.openmdao.densityfilter_comp import DensityFilterComp
...
class FEM2DSimpGroup(Group):
    ...
    def setup(self):
        ...
        self.connect('penalization_comp.y', 'filter_comp.dvs')

        # filter
        comp = DensityFilterComp(length_x=length_x, length_y=length_y, num_nodes_x=num_nodes_x, num_nodes_y=num_nodes_y,
                                 num_dvs=num_el, radius= length_x/(float(num_nodes_x) -1)*2)
        self.add_subsystem('filter_comp', comp)
        # self.connect('filter_comp.dvs_bar', 'penalization_comp_re.x')
        self.connect('filter_comp.dvs_bar', 'states_comp.multipliers')
        
        # # penalization2
        # comp = PenalizationComp(num=num_el, p=p)
        # self.add_subsystem('penalization_comp_re', comp)

        # self.connect('penalization_comp_re.y', 'states_comp.multipliers')
